...
export default function Evals({
    ...
    return (
      <Shell>
        ...
        <p className="empty">
          ...
          in Claude Code. It runs the six gates against the deployed app and
          writes the report this page renders.
        </p>
      </Shell>
    );
  }
      ...
      <Shell>
        {schemaIssue ? (
          <div className="notice bad">
            ...
            <p>
              The shape is <code>EvalsReport</code> in{" "}
              <code>packages/contract</code>. Do not hand-edit the file; re-run
              the eval.
            </p>
  ...
  const pct = rubric.total
    ? Math.round((rubric.awarded / rubric.total) * 100)
    : 0;
  ...
  return (
    <Shell>
      ...
      <div className="score">
        ...
        <span className={`pill ${bench.pass ? "done" : "error"}`}>
          bench {bench.pass ? "pass" : "fail"}
        </span>
        <span
          className={`pill ${quality.errors ? "error" : quality.warnings ? "cap" : "done"}`}
        >
          ...
          <table>
            ...
            <tbody>
              {report.gates.map((g: GateResult) => (
                <tr
                  key={g.gate}
                  className={g.status === "fail" ? "fail" : undefined}
                >
                ...
                <tr key={i} className={r.ok ? undefined : "fail"}>
                  ...
                  <td>
                    {r.ok ? (
                      ...
                      <span className="cross">✗ {r.detail ?? ""}</span>
                    )}
                  </td>
                </tr>
              ))}
          ...
          <tbody>
            {bench.sla.map((r: SlaRow) => (
              <tr key={r.metric} className={r.pass ? undefined : "fail"}>
                ...
                <td className="num">
                  {r.actual === null ? "—" : `${round(r.actual)}${r.unit}`}
                </td>
                <td>
                  {r.pass ? (
                    <span className="tick">✓</span>
  ...
  return (
    <div className="traj">
      <h4>
        ...
        {t.terminated ? (
          <span className={`pill ${t.terminated}`}> {t.terminated}</span>
        ) : null}
      </h4>
          ...
          <div key={s.step} className={`step${s.ok ? "" : " bad"}`}>
            ...
            <span className="why">
              {s.ok ? (
                ...
                <span className="err">{s.error ?? "failed"}</span>
              )}
            </span>
            {s.ms !== undefined ? (
