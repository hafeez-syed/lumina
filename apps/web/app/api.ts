...
/**
 * The browser-side client. It calls this app's own `/api/*` Route Handlers, which proxy to
 * the gateway server-side (see `app/api/[...path]/route.ts`). Same-origin, so there is no
 * base URL to configure and no CORS preflight; the gateway address lives in `GATEWAY_URL`
 * on the server and never reaches the client bundle. The browser still never talks to the
 * agent service, and still never holds a key.
 *
 * NOTE: this module is NOT a route. Only `route.ts` inside `app/` becomes an endpoint.
 * Import this file for the client; never import from `app/api/[...path]/route.ts`, which
 * would drag server code and `GATEWAY_URL` into the browser bundle.
 */
export const API = '/api';

const USER_KEY = 'lumina.userId';

/** X-User-Id is the whole auth story here: one header, required on every route but /health. */
