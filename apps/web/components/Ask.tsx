...
import { api, ApiError, askStream, setUserId, userId } from "@/app/api";
    ...
    <>
      <header className="topbar">
        ...
        <div className="health" title="GET /health">
          <span
            className={`dot ${health?.status === "ok" ? "ok" : health ? "bad" : ""}`}
          />
          {health
        ...
        <div>
          ...
          {!messages.length && !error && (
            <div className="panel" style={{ marginTop: 12 }}>
              ...
              <div className="empty">
                Ask a question and the answer streams in with citations you can
                click. Until the backend implements a route you will see a{" "}
                <code>501 not implemented yet</code> notice instead — that is
            ...
            {messages.map((m, i) =>
              m.role === "user" ? (
                ...
                <div key={i} className="msg">
                  <header>
                    lumina
                    {m.depth === "deep" || m.done?.depth === "deep" ? (
                      <span className="pill deep">deep</span>
                    ) : null}
                    {m.done ? (
                      <span className={`pill ${m.done.terminated}`}>
                        {m.done.terminated}
                      </span>
                    ) : null}
                  </header>
