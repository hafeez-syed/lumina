 ...
 * (which nests the agent service's health), a 501 for every contract route, and optional
 * static hosting of a pre-built UI. That is deliberately the boring half.
 *
...
const notImplemented = (route: string) => (_req: express.Request, res: express.Response) => {
  res.status(501).json({
    error: `not implemented yet: ${route}. Build it in apps/gateway/src/.`,
    status: 501,
