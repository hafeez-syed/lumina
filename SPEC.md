...
| Mongoose schemas drift from the zod contract | zod is the source of truth; the UI compiles against the same types, so drift fails `pnpm check-types` before it fails a learner |
